...
public class TotitoBoard implements Board<Integer> {
    ...
    private static final int NUM_SQR = 9;
    ...
    //Constructor
    public TotitoBoard() {
        position = new TotitoPiece[NUM_SQR];
        turn = TotitoPiece.X;
        Arrays.fill(position,TotitoPiece.E);
    }

    //Deep copy, basically a clone
    ...
    public double evaluate(Piece player) {
        if(isWin() && turn == player) return -1;
        else if(isWin() && turn != player) return 1;
        else return 0.0;
    }
